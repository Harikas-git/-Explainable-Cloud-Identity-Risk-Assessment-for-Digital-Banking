...
try:
    ...
    from backend.context_layer import AccessRequest
    from backend.decision_module import DecisionResult
except ImportError:
    import config
...
class ExplanationLayer:
    ...
    def generate_narratives(
        ...
        if decision_res.decision == "GRANT":
            user_msg = "Access granted. Your session context meets digital banking zero-trust criteria."
        ...
        if getattr(req, "vpn_connected", 1) == 0:
            remediation_steps.append("Connect through corporate encrypted banking VPN (reduces score by ~15 pts)")
